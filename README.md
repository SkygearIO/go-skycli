# go-skycli - Skygear Command Line Interface

`go-skycli` was the old `skycli`, which is obsoleted since we wish to have a new
[skycli](https://github.com/skygeario/skycli) which solely focus in deplyoing Cloud Functions.

If you wish to keep using it for DB schema management, you can still use the binary in this repo.
The `go get` command described below would not works. Plesae use the binary from github if you're
not sure
